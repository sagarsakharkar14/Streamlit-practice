...
# Function toencode_data train the model
@st.cache_resource(show_spinner="Trainig model...")
...
@st.cache_resource(show_spinner='Making a prediction...')
def make_prediction(_model, _X_encoder, X_pred):
    features = [each[0] for each in X_pred]
    features = np.array(features).reshape(1,-1)
    enocoded_feature = _X_encoder.transform(features)
    pred = _model.predict(enocoded_feature)
    return pred[0]
...
if __name__ == "__main__":
    ...
    # 3. Encode the data
        encoded_data = encode_data(df, oe, le)
    # 4. Train the model
    ...
    # 6. Format the prediction to be a nice text
        nice_pre = "The Mushroom is poisonous" if pred==1 else "The Mushroom is edible"
